...
void callback_lidar_twk2(const sensor_msgs::LaserScan::ConstPtr &msg)
{
    ...
    int angle_start[] = {0, 30, 60, 90, 120, 150}; // 우측부터 반시계 방향으로 1,2,3,4,5,6 구역으로 나눔
    float max_radius = 4.0f;                       // 반경값이 0으로 인식될 시, 대신할 값
    float stop_radius = 1.2f;                      // 전방 1.8 meter 이내에 라이다 인식될 시, 멈추게 끔 하기 위해 필요한 변수
   ...
   /* else
    ...
    if (sine >= 0.0f)
    {
        ...
        if( theta > 20 && f_left == 1 )
        {
            PWM_sv = STEERING_LEFT;
            printf("left\n");
        }
        else if (theta > 20 && f_left == 0 )
            //PWM_sv = STEERING_CENTER;
            printf("pass\n");
        else
    ...
    {
        // go right
        if( theta > 20 && f_right == 1 )
        {
            PWM_sv = STEERING_RIGHT;
            printf("right\n");
        }
        else if( theta > 20 && f_right == 0 )
            //PWM_sv = STEERING_CENTER;
            printf("pass\n");          
        else
            PWM_sv = STEERING_CENTER + theta * theta / 5.5;
